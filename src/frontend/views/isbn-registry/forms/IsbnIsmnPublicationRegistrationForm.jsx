...
function IsbnIsmnPublicationRegistrationForm(props) {
  ...
  const handlePublicationRegistration = async values => {
    ...
    async function makeApiCall(turnstileToken) {
      ...
      await createRequest(formattedIsbnRequest, '/api/public/isbn-registry/requests/publications', history, setSnackbarMessage, language);
  ...
  return information ? (
    <div>
      ...
      <RenderTurnstileNotification language={language} setInformation={setInformation} formType='isbnIsmn' />
    </div>
  ) : (
    <Form
